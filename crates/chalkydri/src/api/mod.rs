...
pub(super) async fn calibration_status(data: web::Data<ApiData>) -> impl Responder {
    let cam_man = &data.get_ref().cam_man;
...
#[post("/api/restart")]
pub(super) async fn restart(data: web::Data<ApiData>) -> impl Responder {
    //data.restart().await;
