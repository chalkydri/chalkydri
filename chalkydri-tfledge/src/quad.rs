 ...
 */
fn fit_line(lfps: &[LineFitPoint], sz: i32, i0: i32, i1: i32, double *lineparm, double *err, double *mse){
    //CV_Assert(i0 != i1);
    assert_ne!(i0, i1);
...
int quad_segment_maxima(const DetectorParameters &td, int sz, struct line_fit_pt *lfps, int indices[4]){
    ...
    let ksz = if 20 < (sz/12) { 20 } else { sz/12 };
    ...
    let errs: Vec<f64> = Vec::with_capacity(sz);
    ...
    //if (1) {
    {
        ...
        let y: Vec<f64> = Vec::with_capacity(sz);
        ...
        let mut fsz = libm::floor(libm::sqrt(-log(cutoff)*2*sigma*sigma)) + 1;
        //fsz = 2*fsz + 1;
        fsz = 2*fsz + 1;
        ...
        let f: Vec<f32> = Vec::with_capacity(fsz);

        for i in 0..fsz {
            //int j = i - fsz / 2;
            let j = i - fsz / 2;
            //f[i] = (float)exp(-j*j/(2*sigma*sigma));
            f[i] = exp(-j*j/(2*sigma*sigma)) as f32;
        }
    ...
    let mut maxima: Vec<i32> = Vec::with_capacity(sz);
    //std::vector<double> maxima_errs(sz);
    let mut maxima_errs: Vec<f64> = Vec::with_capacity(sz);
    //int nmaxima = 0;
    let nmaxima = 0i32;
...
int fit_quad(const DetectorParameters &_params, const Mat im, zarray_t *cluster, struct sQuad *quad){
    ...
    for (int pidx = 0; pidx < sz; pidx++) {
        ...
        xmax = if xmax > p->x { xmax } else { p->x };
        xmin = if xmin < p->x { xmin } else { p->x };

        ymax = if ymax > p->y { ymax } else { p->y };
        ymin = if ymin < p->y { ymin } else { p->y };
    }
